...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname mt2-shortans) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; ******************************************
...
(define jumble-list-format (list "volleyball"
...
;; Complete with the list in list format
(define clothes-list-format (list 'shirt
                                  (list 'mittens)
                                  (list 'socks (list 'shoes
                                               'boots 'sandals))
                                  'pants
                                  'dress
                                  'jumper
...
(define clothes-quote-format '(shirt
                               (mittens)
...
(define (clothes-rearrange c)
  (cons (first (rest (first (rest
              (first (rest (rest c)))))))
        (cons (first (rest (rest (rest (rest c)))))
              (rest (rest (rest (rest (rest (rest (rest c))))))))))
...
(define (spreadyint-template n)
   (cond [(= n -5)                             ...]
         [(= n 11)                             ...]
         [else (... n ... (spreadyint-template (/ n 3)))]))
...
;; 'P 'Q 'R 'S 'T 'U 'V
(define thing-invalid-defs (list 'R 'S 'T))