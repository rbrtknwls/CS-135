...
(define (valid-date? a)
  (cond
    [(> (get-month a) 12)                      false]
    [(withinskip? a)                           false]
    [(day-in-boundry (get-month a)
                     (get-year a) (get-day a)) true]
    [else                                      false]))
...
(define (day-in-boundry a b c)
  (cond
    [(= a jan) (<= c 31)] [(= a mar)      (<= c 31)]
    [(= a may) (<= c 31)] [(= a jul)      (<= c 31)]
    [(= a aug) (<= c 31)] [(= a oct)      (<= c 31)]
    [(= a dec) (<= c 31)] [(= a apr)      (<= c 30)]
    [(= a jun) (<= c 30)] [(= a sep)      (<= c 30)]
    [(= a nov) (<= c 30)] [(leap-year? b) (<= c 29)]
    [else      (<= c 28)]))
...
(define (withinskip? a)
  (cond
    [(> (get-year a) 1752)      false]
    [(< (get-year a) 1752)      false]
    [(< (get-month a) sep)      false]
    [(> (get-month a) sep)      false]
    [(> (get-day a) 13)         false]
    [(< (get-day a) 3)          false]
    [else                       true]))
