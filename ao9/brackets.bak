...
;; (compare-brack blist bstack) Given a string of brackets, compares
;;  if for each element in b string its corrisponding value will exist
;;  within the bstack
;; Examples:
(check-expect (compare-brack (list #\)) (list #\()) true)
(check-expect (compare-brack (list #\)) (list #\<)) false)
(check-expect (compare-brack (list #\( #\< #\> #\)) empty) true)
(check-expect (compare-brack (list #\( #\[ #\> #\)) empty) false)
